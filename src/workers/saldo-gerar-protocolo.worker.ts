...
const WORKER = 'worker:saldo-gerar-protocolo'

export async function iniciarWorkerSaldoGerarProtocolo(): Promise<void> {
  const queueUrl = process.env.SQS_SALDO_URL
  if (!queueUrl) {
    console.warn(`[${WORKER}] SQS_SALDO_URL não configurada — worker desativado`)
    return
  }
...
async function processarMensagem(queueUrl: string, receiptHandle: string, rawBody: string): Promise<void> {
  ...
  try {
    ...
    const result = await gerarProtocoloSaldo({
      ...
      valorDivida: Number(input.vlr_divida_atualizada ?? input.valorDivida),
      valorAcordo: Number(input.vlr_acordo_digio ?? input.valorAcordo),
      arquivoOrigem,
    })
    await deletarMensagem(queueUrl, receiptHandle)
    console.log(`[${WORKER}] ✓ cpf=${ofuscarCpf(result.cpf)} protocolo=${result.protocolo}`)
  } catch (err) {
