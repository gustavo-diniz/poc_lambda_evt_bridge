...
// ===========================================================================
// FGO — Fundo de Garantia de Operações de Crédito  (v2)
// ===========================================================================
//
// Cenários controlados por campos do body:
//
// codigoidentificadorExternoOperacao:
//   "ERR401"  → 401 Não autorizado
//   "ERR500"  → 500 Erro interno
//
// codigoFundoGarantidor:
//   99        → 400 Fundo Garantidor não cadastrado
//
// percentualGarantiaOperacaoCredito:
//   valor <= 0 → 400 Percentual inválido
//
// PUT /v2/reservas/cancelamentos — numeroReservaPreValidacao:
//   999999    → 400 Reserva não encontrada
//   999998    → 500 Erro interno
//
// ===========================================================================

let reservaCounter = 100000;

function gerarNumeroReserva() {
  return ++reservaCounter;
}

function fgoCorrelationId() {
  return `correlationId: ${Math.random().toString(16).slice(2, 10)}-${Math.random().toString(16).slice(2, 6)}-${Math.random().toString(16).slice(2, 6)}-${Math.random().toString(16).slice(2, 6)}-${Math.random().toString(16).slice(2, 14)}`;
}

function fgoErro400(res, detail, userHelp) {
  const body = {
    detail,
    instance: fgoCorrelationId(),
    userHelp,
  };
  console.log(`  -> HTTP 400 FGO | ${detail}`);
  return res.status(400).json(body);
}

function fgoErro401(res) {
  const body = {
    statusCode: 401,
    error: 'Unauthorized',
  ...
  console.log(`  -> HTTP 401 FGO | Não autorizado`);
  return res.status(401).json(body);
}

function fgoErro500(res) {
  const body = {
    detail: 'Erro Interno do Servidor',
    instance: fgoCorrelationId(),
    userHelp: 'Tente novamente mais tarde.',
  };
  console.log(`  -> HTTP 500 FGO | Erro interno`);
  return res.status(500).json(body);
}

function fgoValidarCamposComuns(res, body) {
  const { codigoidentificadorExternoOperacao, codigoFundoGarantidor, percentualGarantiaOperacaoCredito } = body ?? {};

  if (codigoidentificadorExternoOperacao === 'ERR401') return fgoErro401(res);
  if (codigoidentificadorExternoOperacao === 'ERR500') return fgoErro500(res);

  if (codigoFundoGarantidor === 99) {
    return fgoErro400(res,
      'O Fundo Garantidor informado não está cadastrado.',
      'Verifique o campo codigoFundoGarantidor informando um código válido (1=FGO Original, 2=FGO PRONAMPE, 3=FGO Desenrola, 4=FGO Procred, 5=FGO Acredita).'
    );
  }

  if (percentualGarantiaOperacaoCredito !== undefined && percentualGarantiaOperacaoCredito <= 0) {
    return fgoErro400(res,
      'O percentual de garantia informado é inválido.',
      'Informe um valor positivo com até duas casas decimais no campo percentualGarantiaOperacaoCredito.'
    );
  }
...
// ---------------------------------------------------------------------------
// POST /v2/pre-validacoes  — Pré-validar SEM reserva
// ---------------------------------------------------------------------------
app.post('/v2/pre-validacoes', (req, res) => {
  console.log('\n[POST] /v2/pre-validacoes (FGO - sem reserva)');
  console.log(`  Body: ${JSON.stringify(req.body)}`);

  const erro = fgoValidarCamposComuns(res, req.body);
  if (erro) return erro;

  const resposta = {
    valorTotalFinanciado: 987451.75,
    valorMargemDisponivelContratacao: 12548.25,
  };
  console.log(`  -> HTTP 200 FGO | pre-validacao sem reserva aceita`);
  return res.status(200).json(resposta);
});

// ---------------------------------------------------------------------------
// POST /v2/pre-validacoes/reservas  — Pré-validar COM reserva
// ---------------------------------------------------------------------------
app.post('/v2/pre-validacoes/reservas', (req, res) => {
  console.log('\n[POST] /v2/pre-validacoes/reservas (FGO - com reserva)');
  console.log(`  Body: ${JSON.stringify(req.body)}`);
  ...
  const resposta = {
    numeroReservaPreValidacao: numeroReserva,
    valorTotalFinanciado: 987451.75,
    valorMargemDisponivelContratacao: 12548.25,
  };
  console.log(`  -> HTTP 200 FGO | reserva gerada: ${numeroReserva}`);
  return res.status(200).json(resposta);
});

// ---------------------------------------------------------------------------
// PUT /v2/reservas/cancelamentos  — Cancelar reserva
// ---------------------------------------------------------------------------
app.put('/v2/reservas/cancelamentos', (req, res) => {
  console.log('\n[PUT] /v2/reservas/cancelamentos (FGO)');
  console.log(`  Body: ${JSON.stringify(req.body)}`);

  const { codigoFundoGarantidor, numeroReservaPreValidacao } = req.body ?? {};

  if (codigoFundoGarantidor === 99) {
    return fgoErro400(res,
      'O Fundo Garantidor informado não está cadastrado.',
      'Verifique o campo codigoFundoGarantidor informando um código válido.'
    );
  }

  if (numeroReservaPreValidacao === 999999) {
    return fgoErro400(res,
      'Reserva de pré-validação não localizada.',
      'Verifique o campo numeroReservaPreValidacao e informe um número de reserva válido.'
    );
  }

  if (numeroReservaPreValidacao === 999998) {
    return fgoErro500(res);
  }

  console.log(`  -> HTTP 204 FGO | reserva ${numeroReservaPreValidacao} cancelada com sucesso`);
...
// GET /fgo/cenarios
// ---------------------------------------------------------------------------
app.get('/fgo/cenarios', (req, res) => {
  res.json({
    descricao: 'Cenários de teste disponíveis no mock da API FGO v2',
    endpoints: {
      'POST /v2/pre-validacoes': 'Pré-validação SEM reserva — retorna valorTotalFinanciado e valorMargemDisponivelContratacao',
      'POST /v2/pre-validacoes/reservas': 'Pré-validação COM reserva — retorna numeroReservaPreValidacao + valores',
      'PUT  /v2/reservas/cancelamentos': 'Cancelamento de reserva — retorna 204 sem corpo',
    },
    cenarios: {
      sucesso: {
        'pre-validacoes (sem reserva)': 'Qualquer body válido → HTTP 200 com valorTotalFinanciado e valorMargemDisponivelContratacao',
        'pre-validacoes/reservas (com reserva)': 'Qualquer body válido → HTTP 200 com numeroReservaPreValidacao gerado automaticamente',
        'reservas/cancelamentos': 'Qualquer body válido → HTTP 204 sem conteúdo',
      },
      erro400: {
        'codigoFundoGarantidor: 99': 'HTTP 400 — Fundo Garantidor não cadastrado (todos os endpoints)',
        'percentualGarantiaOperacaoCredito <= 0': 'HTTP 400 — Percentual inválido (pre-validacoes e pre-validacoes/reservas)',
        'numeroReservaPreValidacao: 999999': 'HTTP 400 — Reserva não localizada (cancelamentos)',
      },
      erro401: {
        'codigoidentificadorExternoOperacao: "ERR401"': 'HTTP 401 — Não autorizado (pre-validacoes e pre-validacoes/reservas)',
      },
      erro500: {
        'codigoidentificadorExternoOperacao: "ERR500"': 'HTTP 500 — Erro interno (pre-validacoes e pre-validacoes/reservas)',
        'numeroReservaPreValidacao: 999998': 'HTTP 500 — Erro interno (cancelamentos)',
      },
    },
    exemplos_curl: {
      sucesso_sem_reserva: 'POST /v2/pre-validacoes com body padrão do swagger',
      sucesso_com_reserva: 'POST /v2/pre-validacoes/reservas com body padrão do swagger',
      cancelar: 'PUT /v2/reservas/cancelamentos com { codigoFundoGarantidor: 1, codigoAgenteFinanceiro: 123, numeroReservaPreValidacao: 123 }',
      erro_fundo_invalido: 'Qualquer endpoint com codigoFundoGarantidor: 99',
      erro_nao_autorizado: 'POST /v2/pre-validacoes com codigoidentificadorExternoOperacao: "ERR401"',
      erro_reserva_nao_encontrada: 'PUT /v2/reservas/cancelamentos com numeroReservaPreValidacao: 999999',
    },
  });
});
...
app.listen(PORT, () => {
  ...
  console.log(`\n  GET    /cenarios      ->  tabela de CPFs de teste (FGTS/Desenrola)`);
  console.log(`\n  FGO v2:`);
  console.log(`  POST   /v2/pre-validacoes`);
  console.log(`  POST   /v2/pre-validacoes/reservas`);
  console.log(`  PUT    /v2/reservas/cancelamentos`);
  console.log(`  GET    /fgo/cenarios  ->  tabela de cenários FGO`);
  console.log('\n' + '='.repeat(60) + '\n');
});
